...
async function renderTransactionsView(container) {
  ...
  function openCreateModal() {
    ...
    const fields = [
      ...
      { name: "credit_card_id", label: "Cartão de crédito (opcional)", type: "select", options: creditCardOptions },
      { name: "category_id", label: "Categoria", type: "select", options: categoryOptionsFor("expense") },
    ...
    typeSelect.addEventListener("change", () => {
      ...
      categoryOptionsFor(typeSelect.value).forEach((opt) => {
  ...
  function openEditModal(t) {
    ...
    const fields = [
      { name: "category_id", label: "Categoria", type: "select", options: categoryOptionsFor(t.type) },
      { name: "description", label: "Descrição", required: true },
      { name: "amount", label: "Valor (R$)", type: "number", step: "0.01", required: true },
      { name: "is_paid", label: "Já efetivada", type: "checkbox" },
      { name: "notes", label: "Notas (opcional)", type: "textarea" },
