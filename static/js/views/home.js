/* Início — resumo de saldo, transações recentes e metas em andamento. */

async function renderHomeView(container) {
  container.appendChild(UI.el("div", { class: "loading" }, "Carregando..."));

  const [accounts, txResp, goals] = await Promise.all([
    Api.accounts.list(),
  ...
  container.appendChild(UI.el("h1", { class: "page-title" }, "Início"));
